...
void NetworkClient::ProcessReply_ControllerData(unsigned int data_size, char * data, unsigned int dev_idx)
{
    ...
    if(data_size == *((unsigned int*)data))
    {
        ...
        {
            server_controllers[dev_idx]->active_profile = new_controller->active_profile;
            server_controllers[dev_idx]->profiles       = new_controller->profiles;
...
void NetworkClient::SendRequest_RGBController_SetDeviceProfile(unsigned int dev_idx, unsigned char *data, unsigned int size)
{
    if(change_in_progress)
    {
        return;
    ...
    InitNetPacketHeader(&request_hdr, dev_idx, NET_PACKET_ID_RGBCONTROLLER_SETDEVICEPROFILE, size);
