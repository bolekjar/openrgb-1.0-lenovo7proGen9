    ...
    ui(new Ui::OpenRGBDevicePageUi)
{
    ...
    if(device->profiles > 0)
    {
        ...
        for (std::size_t i = 0; i < device->profiles; i++)
        {
            ui->ProfileBox->addItem(QString("Profile ").append(QString::number(i + 1)));
        }

        ui->ProfileBox->setCurrentIndex(device->active_profile);
...
{
    /*-----------------------------------------------------*\
    | Read current mode can change by profile               |
    \*-----------------------------------------------------*/
    if(static_cast<unsigned int>(device->GetMode()) < device->modes.size()        &&
        device->GetMode() != ui->ModeBox->currentIndex()
        )
    {
        ui->ModeBox->blockSignals(true);
        ui->ModeBox->setCurrentIndex(device->GetMode());
...
void Ui::OpenRGBDevicePage::ShowDeviceView()
{
    ...
    | device contains at least one LED             mode         |
    \*-----------------------------------------------------*/
...
void Ui::OpenRGBDevicePage::on_ProfileBox_currentIndexChanged(int index)
{
    /*-----------------------------------------------------*\
