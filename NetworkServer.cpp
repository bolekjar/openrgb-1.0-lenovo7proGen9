    ...
    while(server_online == true)
    {
        ...
        switch(header.pkt_id)
        {
            ...
            case NET_PACKET_ID_RGBCONTROLLER_ADDSEGMENT:
                ...
                break;
            case NET_PACKET_ID_RGBCONTROLLER_SETDEVICEPROFILE:
                if(data == NULL)
                {
                    break;
                }

                if((header.pkt_dev_idx < controllers.size()) && (header.pkt_size == sizeof(size_t)))
                {
                    size_t profile_id;

                    memcpy(&profile_id, data, sizeof(size_t));

                    controllers[header.pkt_dev_idx]->SetDeviceProfile(profile_id);
                }
                break;
        }
