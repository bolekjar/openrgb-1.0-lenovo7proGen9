...
unsigned char * RGBController::GetDeviceDescription(unsigned int protocol_version)
{
    ...
    {
        data_size += sizeof(profiles) + sizeof(active_profile);
    }
    ...
    if(protocol_version >= 6)
    {
        memcpy(&data_buf[data_ptr], &profiles, sizeof(profiles));
        data_ptr += sizeof(profiles);

        memcpy(&data_buf[data_ptr], &active_profile, sizeof(active_profile));
        data_ptr += sizeof(active_profile);
    }
...
void RGBController::ReadDeviceDescription(unsigned char* data_buf, unsigned int protocol_version)
{
    ...
    /*---------------------------------------------------------*\
    | Copy in controller profiles data                          |
    \*---------------------------------------------------------*/
    if(protocol_version >= 6)
    {
        memcpy(&profiles,&data_buf[data_ptr], sizeof(profiles));
        data_ptr += sizeof(profiles);

        memcpy(&active_profile,&data_buf[data_ptr], sizeof(active_profile));
        data_ptr += sizeof(active_profile);
    }
...
unsigned char *RGBController::GetProfileDescription(size_t profile)
{
    unsigned char *data_buf = new unsigned char[sizeof(profile)];

    /*---------------------------------------------------------*\
    | Copy in LED index                                         |
    \*---------------------------------------------------------*/
    memcpy(data_buf, &profile, sizeof(profile));


    return(data_buf);
}

void RGBController::SetProfileDescription(unsigned char *data_buf)
{
    size_t profile;
    memcpy(&profile, data_buf, sizeof(profile));
